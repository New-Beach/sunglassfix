...
$('.select-shipping').change(function(){
    var selected_value = $('.select-shipping').val();
    if (selected_value == 'Please Select'){
        $('.shipping-selected').html('None Selected');
        if(!$('.shipping-selected').hasClass('slate-text')){
           $('.shipping-selected').addClass('slate-text'); 
        }
    } else {
        $('.shipping-selected').removeClass('slate-text');
        $('.shipping-selected').html(selected_value);
    }
    updateMobileOrderNote();
});
